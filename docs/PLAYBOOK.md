...
Did NEXUS fail?
  ...
  └── Rate limit hit (Claude exhausted)
        └── Codex (o4-mini) auto-kicks in as fallback — no action needed
              If Codex also fails → wait for Claude reset at 11:30 PM IST → then resume
...
## Provider Settings (configured in config.json)

| Setting | Value | Why |
|---------|-------|-----|
| `aiProvider.strategy` | `fallback` | Claude first, Codex (o4-mini) auto-fallback on rate limit |
| `execute.provider` | `claude sonnet` | Best quality for code writing |
| `execute.fallbackProvider` | `codex o4-mini` | Auto-used when Claude rate-limits or fails |
| `council.maxRounds` | `2` | Saves ~4 API calls per run vs 3 rounds |
| `council.*/fallbackProvider` | `codex o4-mini` | All council roles (proposer, critics, evaluator) fallback to Codex |
| `prReviewCouncil.maxRounds` | `1` | PR review only needs one round |

**How fallback works:** No action needed. When Claude hits a rate limit or returns garbage output, every stage (execute, planning council, PR review council) automatically retries with Codex using your OpenAI API key (`OPENAI_API_KEY` injected from `config.json`).

**Claude daily limit resets at:** `11:30 PM IST`

If both Claude AND Codex fail mid-run → wait for Claude reset → `pnpm run resume -- JCP-XXXXX --from-step=<last-failed-step>`
...
| "garbage=true" in logs | Claude returned non-JSON (hit limit or crashed) | Codex fallback auto-triggers; if both fail → wait for Claude reset at 11:30 PM IST, then resume |
