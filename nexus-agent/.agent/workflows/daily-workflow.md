...
> 3. **NEXUS** (`nexus-agent/`) — reads Jira tickets → writes code → ships ADO PRs automatically
>
...
```bash
# Watch live logs
tail -f <project-dir>/logs/$(date +%Y-%m-%d)/*.log

# Watch the council status in real time (Step 4)
watch -n 2 cat <project-dir>/.pipeline-state/JCP-XXXX/council/status.md
...
echo "Focus on app/models/page.model.js. The issue is in the deleteAvailablePage handler." \
  > <project-dir>/.pipeline-state/JCP-XXXX/council/human-feedback.md

# Inspect artifacts after a run
ls <project-dir>/.pipeline-state/JCP-XXXX/
...
| Sentry duplicate tickets | Same error re-processed | State file at `nexus-agent/.sentry-state/processed.json` |
...
- `nexus-agent/src/sentry/` polls Sentry REST API for unresolved issues per service
- Credentials: `sentry-alert/sentry-config.json` (shared by both CLI and Dr.-Nexus daemon)
- State: `nexus-agent/.sentry-state/processed.json` — tracks which Sentry issue IDs have already been turned into Jira tickets (prevents duplicates across restarts)
...
| File | Purpose |
|------|---------|
| `nexus-agent/config.json` | NEXUS runtime config (Jira, ADO, services, AI, Sentry) |
...
All live in `nexus-agent/config.json → services`:
